...
Y = data %>% select(b1:b6)
set.seed(0)
v.noise = sample(c(TRUE, FALSE), NROW(Y), replace = T, prob = c(3,1)/4)
m = Mclust(Y, G = 12, modelNames="VVV", initialization = list(noise=v.noise))

data = data %>%
  mutate(
    g0 = sprintf('%02d', m$classification),
    post = apply(m$z, 1, max)
  )
...
df = t(m$parameters$mean) %>% data.frame %>% tbl_df %>% 
  setNames(balance_nms) %>%
  mutate(g0 = sprintf('%02d', 1:m$G)) %>% 
...
data$g = factor(cluster_partition(m$z, HP), levels = lvls)
