clean_zeros = function(.data) .data  %>% subset(apply(.data %>% dplyr::select(HCO3, Ca, Cl, Mg, K, Na, SO4), 1, prod) != 0)
clean_non_located = function(.data) .data  %>% subset(complete.cases(.data %>% dplyr::select(est, nord)))
compo = function(.data) .data %>% dplyr::select(HCO3, Ca, Cl, Mg, K, Na, SO4) %>% subset(apply(., 1, prod) != 0)
...
balance = function(.data) .data %>% dplyr::select(HCO3, Ca, Cl, Mg, K, Na, SO4) %>%
  mutate(
    ...
    b6 = sqrt(1*1/(1+1)) * log(SO4/Cl) ) %>% dplyr::select(b1,b2,b3,b4,b5,b6)
